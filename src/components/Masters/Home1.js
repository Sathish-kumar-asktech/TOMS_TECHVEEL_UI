...
export default function Home1() {
    ...
    const fetchData = () => {
        axios.get("http://localhost:8081/uom")
    ...
    return (
        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', bgcolor: 'background.default' }}>
            ...
            <Box sx={{display: 'flex',justifyContent: 'space-between',p: 1,m: 1,borderRadius: 1,}}>
                ...
                <Button component={Link} to='/create1' variant="contained" color="primary" sx={{ mb: 2 }}>
                    Add +
                </Button>
                </Box>
                <Table>
                    ...
                    <TableBody>
                        {filteredData.map((d) => (
                            <TableRow key={d.id}>
                                ...
                                <TableCell>{d.product_description}</TableCell>
                                <TableCell>{d.primary}</TableCell>
                                <TableCell>{d.secondary}</TableCell>
                                <TableCell>{d.price}</TableCell>
                                <TableCell>
                                    <Button component={Link} to='/update1' variant="contained" color="primary" size="small" sx={{ mr: 1 }}>
                                        Update
                                    </Button>
